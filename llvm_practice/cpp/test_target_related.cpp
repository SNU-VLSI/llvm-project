...
class TargetRelatedMethodsTest : public testing::Test {
protected:
...
TEST_F(TargetRelatedMethodsTest, test1) {
  Triple triple = TM->getTargetTriple();
  printf("Triple : %s\n", triple.getTriple().c_str());
  printf("Arch : %s\n", triple.getArchName().str().c_str());

  RISCVTargetMachine *riscv_tm = dynamic_cast<RISCVTargetMachine *>(TM.get());
  const RISCVSubtarget &ST = riscv_tm->getSubtarget<RISCVSubtarget>(*F);
  const RISCVRegisterInfo *rinfo = ST.getRegisterInfo();
  const RISCVInstrInfo *iinfo = ST.getInstrInfo();
}
