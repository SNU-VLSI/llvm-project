//===-- INODEAsmParser.cpp - Parse INODE assembly to
// MCInst instructions ----===//
//
...
#include "MCTargetDesc/INODEInstPrinter.h"
...
// instruction
class INODEOperand : public MCParsedAsmOperand {
  enum OperandKind {
    OpKind_Token,
    OpKind_Reg,
    OpKind_Imm,
...
public:
  INODEOperand(OperandKind Kind, SMLoc StartLoc, SMLoc EndLoc)
  ...
  static std::unique_ptr<INODEOperand> createToken(StringRef Str, SMLoc Loc) {
    auto Op = std::make_unique<INODEOperand>(OpKind_Token, Loc, Loc);
    Op->Token = Str;
    return Op;
  }

  static std::unique_ptr<INODEOperand> createReg(unsigned Num, SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<INODEOperand>(OpKind_Reg, StartLoc, EndLoc);
    Op->RegNo = Num;
    return Op;
  }

  static std::unique_ptr<INODEOperand> createImm(const MCExpr *Expr, SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<INODEOperand>(OpKind_Imm, StartLoc, EndLoc);
    Op->Imm = Expr;
    return Op;
  }
  ...
  void print(raw_ostream &OS) const override {
    ...
    case OpKind_Token:
      ...
      break;
    case OpKind_Reg:
      OS << "Reg: " << INODEInstPrinter::getRegisterName(getReg()) << "\n";
      break;
    }
  }
...
class INODEAsmParser : public MCTargetAsmParser {
// Auto-generated instruction matching functions
#define GET_ASSEMBLER_HEADER
#include "INODEGenAsmMatcher.inc"
...
public:
  INODEAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser, const MCInstrInfo &MII,
...
bool INODEAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name, SMLoc NameLoc,
                                     OperandVector &Operands) {
  // First operand in MCInst is instruction mnemonic.
  Operands.push_back(INODEOperand::createToken(Name, NameLoc));
...
bool INODEAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (Parser.getTok().is(AsmToken::Percent)) {
    ...
    if (parseRegister(RegNo, StartLoc, EndLoc,
      ...
      return true;
    Operands.push_back(INODEOperand::createReg(RegNo, StartLoc, EndLoc));
    return false;
  }
  ...
  if (Parser.getTok().is(AsmToken::Integer)) {
    ...
    Operands.push_back(INODEOperand::createImm(Expr, StartLoc, EndLoc));
    return false;
  }
  // Failure
  return true;
...
bool INODEAsmParser::parseRegister(MCRegister &RegNo, SMLoc &StartLoc, SMLoc &EndLoc,
...
ParseStatus INODEAsmParser::tryParseRegister(MCRegister &RegNo, SMLoc &StartLoc,
                                            SMLoc &EndLoc) {
...
bool INODEAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode, OperandVector &Operands,
                                            MCStreamer &Out, uint64_t &ErrorInfo,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((INODEOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeINODEAsmParser() {
  RegisterMCAsmParser<INODEAsmParser> X(getTheINODETarget());
}
