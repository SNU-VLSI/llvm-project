...
// Expand INODE_LONG_BNE to a BNE, JMP, JUMP instruction sequence.
  ...
  TmpInst =
      MCInstBuilder(INODE::INODE_JMP_INST).addExpr(OffsetExpr);
  Binary = getBinaryCodeForInstr(TmpInst, Fixups, STI);
  support::endian::write(CB, Binary, llvm::endianness::big);

  // Emit an unconditional jump to the destination.
  TmpInst =
      MCInstBuilder(INODE::INODE_JMP_INST).addOperand(SrcSymbol);
  Binary = getBinaryCodeForInstr(TmpInst, Fixups, STI);
  support::endian::write(CB, Binary, llvm::endianness::big);
...
void INODEMCCodeEmitter::encodeInstruction(const MCInst &MI, SmallVectorImpl<char> &CB,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  switch (MI.getOpcode()) {
  default:
...
unsigned INODEMCCodeEmitter::getMachineOpValue(const MCInst &MI, const MCOperand &MO,
                                              SmallVectorImpl<MCFixup> &Fixups,
                                              const MCSubtargetInfo &STI) const {
  if (MO.isReg())
