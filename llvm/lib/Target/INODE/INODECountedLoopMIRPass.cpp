//===- INODECountedLoopMIRPass - MIR level targeting of hardware loops -===//
...
// See HardwareLoops + INODELoopConversion for the IR passes that introduces
// the intrinsics that INODEISelLowering lowers to ISD nodes that
// INODEInstrInfo.td lowers to these pseudo instructions.
//
//===----------------------------------------------------------------------===//
...
#include "INODE.h"
// #include "INODECoissueUtil.h"
#include "INODEInstrInfo.h"
#include "INODECountedLoopOptions.h"
...
#define DEBUG_TYPE "INODE-counted-loop-mir"
#define PASS_NAME "INODE Counted Loop MIR Pass"

namespace {
class INODECountedLoopMIR : public MachineFunctionPass {
private:
...
public:
  static char ID;
  explicit INODECountedLoopMIR() : MachineFunctionPass(ID) {}
...
char INODECountedLoopMIR::ID = 0;
INITIALIZE_PASS_BEGIN(INODECountedLoopMIR, DEBUG_TYPE, PASS_NAME, false, false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfoWrapperPass)
INITIALIZE_PASS_END(INODECountedLoopMIR, DEBUG_TYPE, PASS_NAME, false, false)
FunctionPass *llvm::createINODECountedLoopMIRPass() {
  return new INODECountedLoopMIR();
}
...
bool eliminatePseudos(MachineBasicBlock *BB, const TargetInstrInfo &TII) {
  ...
  for (auto BBI = BB->instr_begin(), E = BB->instr_end(); BBI != E;) {
    ...
    case INODE::CLOOP_BEGIN_TERMINATOR: {
      BBI = BB->erase(BBI);
      changed = true;
      break;
    }
...
Error canLowerToBNE(MachineBasicBlock *header, MachineBasicBlock *body) {
  ...
  assert(bodyEndBranch->getOpcode() == INODE::CLOOP_END_BRANCH);
  ...
  for (--MI; MI->getOpcode() != INODE::CLOOP_END_VALUE; --MI) {
...
void lowerToBNE(MachineBasicBlock *header, MachineBasicBlock *body,
  ...
  auto headerBeginValue =
      findBySearchingFromTerminator(header, INODE::CLOOP_BEGIN_VALUE);
  auto headerBeginTerminator = header->getFirstInstrTerminator();
  auto bodyEndValue =
      findBySearchingFromTerminator(body, INODE::CLOOP_END_VALUE);
  auto bodyEndBranch = body->getFirstInstrTerminator();
  ...
  // } else if (opcode == INODE::ADD_SI || opcode == INODE::ADD_ZI) {
  //   assert(InstrBeforeCLoopBegin->getOperand(0).isReg() &&
  //          InstrBeforeCLoopBegin->getOperand(0).getReg() == reg);
...
MachineBasicBlock *findMatchingEndVal(MachineInstr *BeginTerm,
  ...
  assert(BeginTerm && BeginTerm->getOpcode() == INODE::CLOOP_BEGIN_TERMINATOR &&
  ...
  for (; BeginValDN; BeginValDN = BeginValDN->getIDom()) {
    if (!containsPseudos(BeginValDN->getBlock(), {INODE::CLOOP_BEGIN_VALUE}))
      continue;
    auto BeginValI = findBySearchingFromTerminator(BeginValDN->getBlock(),
                                                   INODE::CLOOP_BEGIN_VALUE);
...
bool INODECountedLoopMIR::cleanPrologs(MachineBasicBlock *Preheader,
                                      MachineBasicBlock *EndValueBB) {
...
MachineBasicBlock *INODECountedLoopMIR::getPreheader(MachineLoop &L,
                                                    bool &isPipelined) {
  MachineBasicBlock *preheader = L.getLoopPreheader();
  if (!containsPseudos(preheader, {INODE::CLOOP_BEGIN_TERMINATOR}))
    isPipelined = true;
...
bool INODECountedLoopMIR::traverseLoop(MachineLoop &L) {
  bool changed = false;
  ...
  for (MachineBasicBlock *MBB : Latches) {
    if (containsPseudos(MBB, {INODE::CLOOP_END_BRANCH})) {
      EndBranchBB = MBB;
  ...
  auto bodyEndValue =
      findBySearchingFromTerminator(EndBranchBB, INODE::CLOOP_END_VALUE);

  if (!containsPseudos(
          Preheader, {INODE::CLOOP_BEGIN_TERMINATOR, INODE::CLOOP_BEGIN_VALUE}) ||
  ...
  auto headerBeginValue =
      findBySearchingFromTerminator(Preheader, INODE::CLOOP_BEGIN_VALUE);
  ...
  auto cloopValueRegistersMatch = [](MachineBasicBlock::instr_iterator i) {
    ...
    assert(i->getOpcode() == INODE::CLOOP_BEGIN_VALUE ||
           i->getOpcode() == INODE::CLOOP_END_VALUE);
    if (i->getOpcode() == INODE::CLOOP_BEGIN_VALUE)
...
bool INODECountedLoopMIR::runOnMachineFunction(MachineFunction &mf) {
  auto &ST = mf.getSubtarget<INODESubtarget>();
  TII = ST.getInstrInfo();

  if (!INODE::CountedLoop::EnableMIR)
    return false;

  LLVM_DEBUG(dbgs() << "INODE counted loop mir on function " << mf.getName()
                    << "\n";);
