...
M88kTargetLowering::M88kTargetLowering(const TargetMachine &TM, const M88kSubtarget &STI)
...
SDValue M88kTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
...
SDValue performANDCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  if (isShiftedMask(Mask->getZExtValue(), MaskWidth, MaskOffset)) {
    if (FirstOperandOpc == ISD::SRL || FirstOperandOpc == ISD::SRA) {
      ...
      // immediate.
      ConstantSDNode *ShiftAmt = dyn_cast<ConstantSDNode>(FirstOperand.getOperand(1));
      if (!(ShiftAmt))
        return SDValue();
      ...
      return SDValue();
  } else if (isShiftedMask(~Mask->getZExtValue() & ((0x1ULL << ValTy.getSizeInBits()) - 1),
...
SDValue M88kTargetLowering::PerformDAGCombine(SDNode *N, DAGCombinerInfo &DCI) const {
...
SDValue M88kTargetLowering::LowerFormalArguments(SDValue Chain, CallingConv::ID CallConv,
                                                 bool IsVarArg,
...
SDValue M88kTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
