...
static cl::opt<bool>
    EnableMemCpyDAGOpt("enable-memcpy-dag-opt", cl::Hidden, cl::init(true),
                       cl::desc("Gang up loads and stores generated by inlining of memcpy"));

static cl::opt<int> MaxLdStGlue("ldstmemcpy-glue-max",
                                cl::desc("Number limit for gluing ld/st of memcpy."), cl::Hidden,
                                cl::init(0));
...
bool ConstantFPSDNode::isValueValidForType(EVT VT, const APFloat &Val) {
  ...
  bool losesInfo;
  (void)Val2.convert(SelectionDAG::EVTToAPFloatSemantics(VT), APFloat::rmNearestTiesToEven,
                     &losesInfo);
  return !losesInfo;
}
...
bool ISD::isConstantSplatVector(const SDNode *N, APInt &SplatVal) {
  ...
  bool HasUndefs;
  unsigned EltSize = N->getValueType(0).getVectorElementType().getSizeInBits();
  return BV->isConstantSplat(SplatVal, SplatUndef, SplatBitSize, HasUndefs, EltSize) &&
...
bool ISD::matchUnaryPredicate(SDValue Op, std::function<bool(ConstantSDNode *)> Match,
                              bool AllowUndefs) {
...
bool ISD::matchBinaryPredicate(SDValue LHS, SDValue RHS,
  ...
  if (LHS.getOpcode() != RHS.getOpcode() ||
      (LHS.getOpcode() != ISD::BUILD_VECTOR && LHS.getOpcode() != ISD::SPLAT_VECTOR))
    return false;
  ...
  for (unsigned i = 0, e = LHS.getNumOperands(); i != e; ++i) {
    ...
    if (!AllowTypeMismatch &&
        (LHSOp.getValueType() != SVT || LHSOp.getValueType() != RHSOp.getValueType()))
      return false;
...
static void AddNodeIDCustom(FoldingSetNodeID &ID, const SDNode *N) {
  ...
  case ISD::VECTOR_SHUFFLE: {
    ...
    for (unsigned i = 0, e = N->getValueType(0).getVectorNumElements(); i != e; ++i)
...
static void VerifySDNode(SDNode *N) {
  ...
  case ISD::BUILD_VECTOR: {
    ...
    for (const SDUse &Op : N->ops()) {
      assert((Op.getValueType() == EltVT || (EltVT.isInteger() && Op.getValueType().isInteger() &&
...
bool SelectionDAG::RemoveNodeFromCSEMaps(SDNode *N) {
  ...
  case ISD::TargetExternalSymbol: {
    ...
    Erased = TargetExternalSymbols.erase(
        std::pair<std::string, unsigned>(ESN->getSymbol(), ESN->getTargetFlags()));
    break;
  }
  case ISD::MCSymbol: {
  ...
  // not subject to CSE.
  if (!Erased && N->getValueType(N->getNumValues() - 1) != MVT::Glue && !N->isMachineOpcode() &&
      !doNotCSE(N)) {
    N->dump(this);
    dbgs() << "\n";
...
Align SelectionDAG::getEVTAlign(EVT VT) const {
  Type *Ty = VT == MVT::iPTR ? PointerType::get(Type::getInt8Ty(*getContext()), 0)
...
void SelectionDAG::init(MachineFunction &NewMF, OptimizationRemarkEmitter &NewORE, Pass *PassPtr,
                        const TargetLibraryInfo *LibraryInfo, UniformityInfo *NewUA,
...
SDNode *SelectionDAG::FindNodeOrInsertPos(const FoldingSetNodeID &ID, const SDLoc &DL,
                                          void *&InsertPos) {
  SDNode *N = CSEMap.FindNodeOrInsertPos(ID, InsertPos);
  if (N) {
    switch (N->getOpcode()) {
...
void SelectionDAG::clear() {
  ...
  MCSymbols.clear();
  SDEI.clear();
  std::fill(CondCodeNodes.begin(), CondCodeNodes.end(), static_cast<CondCodeSDNode *>(nullptr));
  std::fill(ValueTypeNodes.begin(), ValueTypeNodes.end(), static_cast<SDNode *>(nullptr));
...
std::pair<SDValue, SDValue> SelectionDAG::getStrictFPExtendOrRound(SDValue Op, SDValue Chain,
...
SDValue SelectionDAG::getVPLogicalNOT(const SDLoc &DL, SDValue Val, SDValue Mask, SDValue EVL,
                                      EVT VT) {
  SDValue TrueValue = getBoolConstant(true, DL, VT, VT);
  return getNode(ISD::VP_XOR, DL, VT, Val, TrueValue, Mask, EVL);
}

SDValue SelectionDAG::getVPPtrExtOrTrunc(const SDLoc &DL, EVT VT, SDValue Op, SDValue Mask,
                                         SDValue EVL) {
  return getVPZExtOrTrunc(DL, VT, Op, Mask, EVL);
}

SDValue SelectionDAG::getVPZExtOrTrunc(const SDLoc &DL, EVT VT, SDValue Op, SDValue Mask,
                                       SDValue EVL) {
...
SDValue SelectionDAG::getConstant(uint64_t Val, const SDLoc &DL, EVT VT, bool isT, bool isO) {
  ...
  assert(
      (EltVT.getSizeInBits() >= 64 || (uint64_t)((int64_t)Val >> EltVT.getSizeInBits()) + 1 < 2) &&
...
SDValue SelectionDAG::getConstant(const ConstantInt &Val, const SDLoc &DL, EVT VT, bool isT,
                                  bool isO) {
  ...
  if (VT.isVector() &&
      TLI->getTypeAction(*getContext(), EltVT) == TargetLowering::TypePromoteInteger) {
  ...
  // only legalize if the DAG tells us we must produce legal types.
    ...
    if (VT.isScalableVector()) {
      ...
      for (unsigned i = 0; i != Parts; ++i)
        ScalarParts.push_back(getConstant(
            NewVal.extractBits(ViaEltSizeInBits, i * ViaEltSizeInBits), DL, ViaEltVT, isT, isO));
    ...
    for (unsigned i = 0; i < ViaVecNumElts / VT.getVectorNumElements(); ++i)
      EltParts.push_back(getConstant(NewVal.extractBits(ViaEltSizeInBits, i * ViaEltSizeInBits), DL,
                                     ViaEltVT, isT, isO));
...
SDValue SelectionDAG::getConstantFP(double Val, const SDLoc &DL, EVT VT, bool isTarget) {
  ...
  if (EltVT == MVT::f80 || EltVT == MVT::f128 || EltVT == MVT::ppcf128 || EltVT == MVT::f16 ||
      EltVT == MVT::bf16) {
    bool Ignored;
    APFloat APF = APFloat(Val);
    APF.convert(EVTToAPFloatSemantics(EltVT), APFloat::rmNearestTiesToEven, &Ignored);
    return getConstantFP(APF, DL, VT, isTarget);
  }
  llvm_unreachable("Unsupported type in getConstantFP");
}

SDValue SelectionDAG::getGlobalAddress(const GlobalValue *GV, const SDLoc &DL, EVT VT,
                                       int64_t Offset, bool isTargetGA, unsigned TargetFlags) {
  ...
  auto *N = newSDNode<GlobalAddressSDNode>(Opc, DL.getIROrder(), DL.getDebugLoc(), GV, VT, Offset,
                                           TargetFlags);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  return SDValue(N, 0);
...
SDValue SelectionDAG::getConstantPool(const Constant *C, EVT VT, MaybeAlign Alignment, int Offset,
                                      bool isTarget, unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTarget) && "Cannot set target flags on target-independent globals");
  if (!Alignment)
    Alignment = shouldOptForSize() ? getDataLayout().getABITypeAlign(C->getType())
  ...
  auto *N = newSDNode<ConstantPoolSDNode>(isTarget, C, VT, Offset, *Alignment, TargetFlags);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
...
SDValue SelectionDAG::getConstantPool(MachineConstantPoolValue *C, EVT VT, MaybeAlign Alignment,
                                      int Offset, bool isTarget, unsigned TargetFlags) {
  assert((TargetFlags == 0 || isTarget) && "Cannot set target flags on target-independent globals");
  if (!Alignment)
...
SDValue SelectionDAG::getTargetIndex(int Index, EVT VT, int64_t Offset, unsigned TargetFlags) {
  FoldingSetNodeID ID;
...
SDValue SelectionDAG::getValueType(EVT VT) {
  ...
  SDNode *&N =
      VT.isExtended() ? ExtendedValueTypeNodes[VT] : ValueTypeNodes[VT.getSimpleVT().SimpleTy];
...
SDValue SelectionDAG::getStepVector(const SDLoc &DL, EVT ResVT, APInt StepVal) {
  ...
  if (ResVT.isScalableVector())
    return getNode(ISD::STEP_VECTOR, DL, ResVT,
...
SDValue SelectionDAG::getVectorShuffle(EVT VT, const SDLoc &dl, SDValue N1, SDValue N2,
                                       ArrayRef<int> Mask) {
  ...
  auto *N = newSDNode<ShuffleVectorSDNode>(VT, dl.getIROrder(), dl.getDebugLoc(), MaskAlloc);
...
SDValue SelectionDAG::getLabelNode(unsigned Opcode, const SDLoc &dl, SDValue Root,
                                   MCSymbol *Label) {
  FoldingSetNodeID ID;
  SDValue Ops[] = {Root};
...
SDValue SelectionDAG::getBlockAddress(const BlockAddress *BA, EVT VT, int64_t Offset, bool isTarget,
...
SDValue SelectionDAG::getAddrSpaceCast(const SDLoc &dl, EVT VT, SDValue Ptr, unsigned SrcAS,
                                       unsigned DestAS) {
  SDValue Ops[] = {Ptr};
  FoldingSetNodeID ID;
  ...
  auto *N = newSDNode<AddrSpaceCastSDNode>(dl.getIROrder(), dl.getDebugLoc(), VT, SrcAS, DestAS);
  createOperands(N, Ops);
...
SDValue SelectionDAG::expandVAArg(SDNode *Node) {
  ...
  SDValue VAListLoad =
      getLoad(TLI.getPointerTy(getDataLayout()), dl, Tmp1, Tmp2, MachinePointerInfo(V));
  ...
  Tmp1 = getNode(ISD::ADD, dl, VAList.getValueType(), VAList,
                 getConstant(getDataLayout().getTypeAllocSize(VT.getTypeForEVT(*getContext())), dl,
                             VAList.getValueType()));
...
SDValue SelectionDAG::expandVACopy(SDNode *Node) {
  ...
  SDValue Tmp1 = getLoad(TLI.getPointerTy(getDataLayout()), dl, Node->getOperand(0),
...
SDValue SelectionDAG::CreateStackTemporary(TypeSize Bytes, Align Alignment) {
  ...
  int FrameIdx =
      MFI.CreateStackObject(Bytes.getKnownMinValue(), Alignment, false, nullptr, StackID);
...
SDValue SelectionDAG::FoldSetCC(EVT VT, SDValue N1, SDValue N2, ISD::CondCode Cond,
                                const SDLoc &dl) {
  EVT OpVT = N1.getValueType();

  auto GetUndefBooleanConstant = [&]() {
    if (VT.getScalarType() == MVT::i1 ||
        TLI->getBooleanContents(OpVT) == TargetLowering::UndefinedBooleanContent)
      return getUNDEF(VT);
  ...
  if (N1CFP && N2CFP) {
    ...
    case ISD::SETNE:
      ...
      [[fallthrough]];
    case ISD::SETONE:
      return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpLessThan, dl, VT,
                             OpVT);
    case ISD::SETLT:
      if (R == APFloat::cmpUnordered)
        return GetUndefBooleanConstant();
    ...
    case ISD::SETLE:
      ...
      [[fallthrough]];
    case ISD::SETOLE:
      return getBoolConstant(R == APFloat::cmpLessThan || R == APFloat::cmpEqual, dl, VT, OpVT);
    case ISD::SETGE:
      if (R == APFloat::cmpUnordered)
        return GetUndefBooleanConstant();
      [[fallthrough]];
    case ISD::SETOGE:
      return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpEqual, dl, VT, OpVT);
    case ISD::SETO:
      return getBoolConstant(R != APFloat::cmpUnordered, dl, VT, OpVT);
    case ISD::SETUO:
      return getBoolConstant(R == APFloat::cmpUnordered, dl, VT, OpVT);
    case ISD::SETUEQ:
      return getBoolConstant(R == APFloat::cmpUnordered || R == APFloat::cmpEqual, dl, VT, OpVT);
    case ISD::SETUNE:
      return getBoolConstant(R != APFloat::cmpEqual, dl, VT, OpVT);
    case ISD::SETULT:
      return getBoolConstant(R == APFloat::cmpUnordered || R == APFloat::cmpLessThan, dl, VT, OpVT);
    case ISD::SETUGT:
      return getBoolConstant(R == APFloat::cmpGreaterThan || R == APFloat::cmpUnordered, dl, VT,
                             OpVT);
    case ISD::SETULE:
      return getBoolConstant(R != APFloat::cmpGreaterThan, dl, VT, OpVT);
    case ISD::SETUGE:
...
bool SelectionDAG::MaskedValueIsZero(SDValue V, const APInt &Mask, const APInt &DemandedElts,
...
APInt SelectionDAG::computeVectorKnownZeroElements(SDValue Op, const APInt &DemandedElts,
                                                   unsigned Depth) const {
...
bool SelectionDAG::isSplatValue(SDValue V, const APInt &DemandedElts, APInt &UndefElts,
                                unsigned Depth) const {
  ...
  case ISD::SPLAT_VECTOR:
    UndefElts = V.getOperand(0).isUndef() ? APInt::getAllOnes(DemandedElts.getBitWidth())
  ...
  case ISD::VECTOR_SHUFFLE: {
    ...
    auto CheckSplatSrc = [&](SDValue Src, const APInt &SrcElts) {
      ...
      return (SrcElts.popcount() == 1) ||
             (isSplatValue(Src, SrcElts, SrcUndefs, Depth + 1) && (SrcElts & SrcUndefs).isZero());
    };
...
const APInt *SelectionDAG::getValidShiftAmountConstant(SDValue V, const APInt &DemandedElts) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) &&
         "Unknown shift node");
...
const APInt *SelectionDAG::getValidMinimumShiftAmountConstant(SDValue V,
...
const APInt *SelectionDAG::getValidMaximumShiftAmountConstant(SDValue V,
                                                              const APInt &DemandedElts) const {
  assert((V.getOpcode() == ISD::SHL || V.getOpcode() == ISD::SRL || V.getOpcode() == ISD::SRA) &&
         "Unknown shift node");
  if (const APInt *ValidAmt = getValidShiftAmountConstant(V, DemandedElts))
...
KnownBits SelectionDAG::computeKnownBits(SDValue Op, unsigned Depth) const {
  ...
  // that all lanes in a scalable vector are considered demanded.
  APInt DemandedElts =
      VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
...
KnownBits SelectionDAG::computeKnownBits(SDValue Op, const APInt &DemandedElts,
  ...
  case ISD::MERGE_VALUES:
    return computeKnownBits(Op.getOperand(Op.getResNo()), DemandedElts, Depth + 1);
  case ISD::SPLAT_VECTOR: {
  ...
  case ISD::MUL: {
    ...
    if (SelfMultiply)
      SelfMultiply &=
          isGuaranteedNotToBeUndefOrPoison(Op.getOperand(0), DemandedElts, false, Depth + 1);
    ...
    if (Op->getFlags().hasNoSignedWrap() && Op.getOperand(0) == Op.getOperand(1) &&
        !Known.isNegative())
  ...
  case ISD::SHL:
    ...
    if (const APInt *ShMinAmt = getValidMinimumShiftAmountConstant(Op, DemandedElts))
  ...
  case ISD::FSHR:
    if (ConstantSDNode *C = isConstOrConstSplat(Op.getOperand(2), DemandedElts)) {
      ...
      if (Amt == 0) {
        Known =
            computeKnownBits(Op.getOperand(Opcode == ISD::FSHL ? 0 : 1), DemandedElts, Depth + 1);
        break;
      }
  ...
  case ISD::SUB: {
    ...
    Known = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
    Known2 = computeKnownBits(Op.getOperand(1), DemandedElts, Depth + 1);
    Known = KnownBits::computeForAddSub(Op.getOpcode() == ISD::ADD, Flags.hasNoSignedWrap(), Known,
                                        Known2);
    break;
  }
  case ISD::USUBO:
  ...
  case ISD::SUBC: {
    ...
    Known = computeKnownBits(Op.getOperand(0), DemandedElts, Depth + 1);
    Known2 = computeKnownBits(Op.getOperand(1), DemandedElts, Depth + 1);
    Known = KnownBits::computeForAddSub(/* Add */ false, /* NSW */ false, Known, Known2);
    break;
  }
  case ISD::UADDO:
  case ISD::SADDO:
  ...
  case ISD::ATOMIC_LOAD_UMAX:
  case ISD::ATOMIC_LOAD: {
    unsigned MemBits = cast<AtomicSDNode>(Op)->getMemoryVT().getScalarSizeInBits();
  ...
  case ISD::FrameIndex:
  case ISD::TargetFrameIndex:
    TLI->computeKnownBitsForFrameIndex(cast<FrameIndexSDNode>(Op)->getIndex(), Known,
                                       getMachineFunction());
    break;
...
unsigned SelectionDAG::ComputeNumSignBits(SDValue Op, const APInt &DemandedElts,
  ...
  case ISD::AssertZext:
    ...
    return VTBits - Tmp;
  case ISD::MERGE_VALUES:
    return ComputeNumSignBits(Op.getOperand(Op.getResNo()), DemandedElts, Depth + 1);
  case ISD::SPLAT_VECTOR: {
  ...
  case ISD::SRA:
    ...
    if (const APInt *ShAmt = getValidMinimumShiftAmountConstant(Op, DemandedElts))
      Tmp = std::min<uint64_t>(Tmp + ShAmt->getZExtValue(), VTBits);
    return Tmp;
  case ISD::SHL:
    if (const APInt *ShAmt = getValidMaximumShiftAmountConstant(Op, DemandedElts)) {
...
unsigned SelectionDAG::ComputeMaxSignificantBits(SDValue Op, const APInt &DemandedElts,
                                                 unsigned Depth) const {
...
bool SelectionDAG::isGuaranteedNotToBeUndefOrPoison(SDValue Op, bool PoisonOnly,
  ...
  APInt DemandedElts = VT.isVector() ? APInt::getAllOnes(VT.getVectorNumElements()) : APInt(1, 1);
  return isGuaranteedNotToBeUndefOrPoison(Op, DemandedElts, PoisonOnly, Depth);
}

bool SelectionDAG::isGuaranteedNotToBeUndefOrPoison(SDValue Op, const APInt &DemandedElts,
  ...
  default:
    ...
    if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN ||
        Opcode == ISD::INTRINSIC_W_CHAIN || Opcode == ISD::INTRINSIC_VOID)
      return TLI->isGuaranteedNotToBeUndefOrPoisonForTargetNode(Op, DemandedElts, *this, PoisonOnly,
                                                                Depth);
    break;
  }
...
bool SelectionDAG::canCreateUndefOrPoison(SDValue Op, bool PoisonOnly, bool ConsiderFlags,
                                          unsigned Depth) const {
  // TODO: Assume we don't know anything for now.
  EVT VT = Op.getValueType();
  if (VT.isScalableVector())
...
bool SelectionDAG::canCreateUndefOrPoison(SDValue Op, const APInt &DemandedElts, bool PoisonOnly,
  ...
  case ISD::MUL:
    ...
    return ConsiderFlags &&
           (Op->getFlags().hasNoSignedWrap() || Op->getFlags().hasNoUnsignedWrap());
  ...
  default:
    ...
    if (Opcode >= ISD::BUILTIN_OP_END || Opcode == ISD::INTRINSIC_WO_CHAIN ||
        Opcode == ISD::INTRINSIC_W_CHAIN || Opcode == ISD::INTRINSIC_VOID)
      return TLI->canCreateUndefOrPoisonForTargetNode(Op, DemandedElts, *this, PoisonOnly,
                                                      ConsiderFlags, Depth);
    break;
  }
...
bool SelectionDAG::isKnownNeverZero(SDValue Op, unsigned Depth) const {
  ...
  case ISD::SUB: {
    ...
    std::optional<bool> ne = KnownBits::ne(computeKnownBits(Op.getOperand(0), Depth + 1),
...
SDValue llvm::getBitwiseNotOperand(SDValue V, SDValue Mask, bool AllowUndefs) {
  ...
  if (ExtArg.getScalarValueSizeInBits() >= MaskC->getAPIntValue().getActiveBits() &&
...
/// vector.
static SDValue foldCONCAT_VECTORS(const SDLoc &DL, EVT VT, ArrayRef<SDValue> Ops,
                                  SelectionDAG &DAG) {
  assert(!Ops.empty() && "Can't concatenate an empty list of vectors!");
  assert(
      llvm::all_of(Ops, [Ops](SDValue Op) { return Ops[0].getValueType() == Op.getValueType(); }) &&
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT) {
  ...
  auto *N = newSDNode<SDNode>(Opcode, DL.getIROrder(), DL.getDebugLoc(), getVTList(VT));
  CSEMap.InsertNode(N, IP);
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT, SDValue N1,
  ...
  if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(N1)) {
    ...
    default:
      break;
    case ISD::SIGN_EXTEND:
      return getConstant(Val.sextOrTrunc(VT.getSizeInBits()), DL, VT, C->isTargetOpcode(),
                         C->isOpaque());
    case ISD::TRUNCATE:
      if (C->isOpaque())
        break;
      [[fallthrough]];
    case ISD::ZERO_EXTEND:
      return getConstant(Val.zextOrTrunc(VT.getSizeInBits()), DL, VT, C->isTargetOpcode(),
                         C->isOpaque());
    case ISD::ANY_EXTEND:
    ...
    case ISD::UINT_TO_FP:
    case ISD::SINT_TO_FP: {
      APFloat apf(EVTToAPFloatSemantics(VT), APInt::getZero(VT.getSizeInBits()));
      (void)apf.convertFromAPInt(Val, Opcode == ISD::SINT_TO_FP, APFloat::rmNearestTiesToEven);
    ...
    case ISD::BITREVERSE:
      return getConstant(Val.reverseBits(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::BSWAP:
      return getConstant(Val.byteSwap(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::CTPOP:
      return getConstant(Val.popcount(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::CTLZ:
    case ISD::CTLZ_ZERO_UNDEF:
      return getConstant(Val.countl_zero(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::CTTZ:
    case ISD::CTTZ_ZERO_UNDEF:
      return getConstant(Val.countr_zero(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::FP16_TO_FP:
    case ISD::BF16_TO_FP: {
      bool Ignored;
      ...
      (void)FPV.convert(EVTToAPFloatSemantics(VT), APFloat::rmNearestTiesToEven, &Ignored);
  ...
  if (ConstantFPSDNode *C = dyn_cast<ConstantFPSDNode>(N1)) {
    ...
    case ISD::FP_EXTEND: {
      ...
      // This can return overflow, underflow, or inexact; we don't care.
      // FIXME need to be more flexible about rounding mode.
      (void)V.convert(EVTToAPFloatSemantics(VT), APFloat::rmNearestTiesToEven, &ignored);
      return getConstantFP(V, DL, VT);
    }
    case ISD::FP_TO_SINT:
    case ISD::FP_TO_UINT: {
  ...
  case ISD::STEP_VECTOR:
    ...
    assert(OpOpcode == ISD::TargetConstant && VT.getVectorElementType() == N1.getValueType() &&
  ...
  case ISD::FP_EXTEND:
    ...
    assert((!VT.isVector() ||
            VT.getVectorElementCount() == N1.getValueType().getVectorElementCount()) &&
  ...
  case ISD::SCALAR_TO_VECTOR:
    ...
    if (OpOpcode == ISD::EXTRACT_VECTOR_ELT && isa<ConstantSDNode>(N1.getOperand(1)) &&
        N1.getConstantOperandVal(1) == 0 && N1.getOperand(0).getValueType() == VT)
      return N1.getOperand(0);
    break;
  case ISD::FNEG:
...
static std::optional<APInt> FoldValueWithUndef(unsigned Opcode, const APInt &C1, bool IsUndef1,
                                               const APInt &C2, bool IsUndef2) {
...
SDValue SelectionDAG::FoldSymbolOffset(unsigned Opcode, EVT VT, const GlobalAddressSDNode *GA,
                                       const SDNode *N2) {
...
SDValue SelectionDAG::FoldConstantArithmetic(unsigned Opcode, const SDLoc &DL, EVT VT,
  ...
  if (NumOps == 2 && VT.isFixedLengthVector() && VT.isInteger() && Ops[0].getValueType() == VT &&
      Ops[1].getValueType() == VT && Ops[0].getOpcode() == ISD::BITCAST &&
    ...
    if (BV1 && BV2 && BVVT.isInteger() && BVVT == N2.getValueType()) {
      ...
      if (BV1->getConstantRawBits(IsLE, EltBits, RawBits1, UndefElts1) &&
        ...
        for (unsigned I = 0, E = NumElts.getFixedValue(); I != E; ++I) {
          std::optional<APInt> Fold =
              FoldValueWithUndef(Opcode, RawBits1[I], UndefElts1[I], RawBits2[I], UndefElts2[I]);
          if (!Fold)
            break;
        ...
        if (RawBits.size() == NumElts.getFixedValue()) {
          ...
          BitVector DstUndefs;
          BuildVectorSDNode::recastRawBits(IsLE, BVVT.getScalarSizeInBits(), DstBits, RawBits,
                                           DstUndefs, BitVector(RawBits.size(), false));
  ...
  if ((Opcode == ISD::MUL || Opcode == ISD::SHL) && Ops[0].getOpcode() == ISD::STEP_VECTOR) {
    ...
    if (ISD::isConstantSplatVector(Ops[1].getNode(), RHSVal)) {
      APInt NewStep = Opcode == ISD::MUL ? Ops[0].getConstantOperandAPInt(0) * RHSVal
...
SDValue SelectionDAG::foldConstantFPMath(unsigned Opcode, const SDLoc &DL, EVT VT, SDValue N1,
                                         SDValue N2) {
  ...
  if (N1CFP && Opcode == ISD::FP_ROUND) {
    ...
    // This can return overflow, underflow, or inexact; we don't care.
    // FIXME need to be more flexible about rounding mode.
    (void)C1.convert(EVTToAPFloatSemantics(VT), APFloat::rmNearestTiesToEven, &Unused);
    return getConstantFP(C1, DL, VT);
  }
...
SDValue SelectionDAG::getAssertAlign(const SDLoc &DL, SDValue Val, Align A) {
  ...
  auto *N = newSDNode<AssertAlignSDNode>(DL.getIROrder(), DL.getDebugLoc(), Val.getValueType(), A);
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT, SDValue N1, SDValue N2,
                              const SDNodeFlags Flags) {
  assert(N1.getOpcode() != ISD::DELETED_NODE && N2.getOpcode() != ISD::DELETED_NODE &&
  ...
  default:
    break;
  case ISD::TokenFactor:
    assert(VT == MVT::Other && N1.getValueType() == MVT::Other && N2.getValueType() == MVT::Other &&
  ...
  case ISD::FP_ROUND:
    assert(VT.isFloatingPoint() && N1.getValueType().isFloatingPoint() &&
           ...
           (N2C->getZExtValue() == 0 || N2C->getZExtValue() == 1) && "Invalid FP_ROUND!");
    if (N1.getValueType() == VT)
      return N1; // noop conversion.
    break;
  case ISD::AssertSext:
  case ISD::AssertZext: {
    ...
    assert(!EVT.isVector() && "AssertSExt/AssertZExt type should be the vector element type "
  ...
  case ISD::EXTRACT_VECTOR_ELT:
    ...
    if (N2C && N1.getOperand(0).getValueType().isFixedLengthVector() &&
      ...
      return getNode(ISD::EXTRACT_VECTOR_ELT, DL, VT, N1.getOperand(N2C->getZExtValue() / Factor),
  ...
  case ISD::EXTRACT_ELEMENT:
    ...
    assert(!N1.getValueType().isVector() && !VT.isVector() &&
           (N1.getValueType().isInteger() == VT.isInteger()) && N1.getValueType() != VT &&
  ...
  case ISD::EXTRACT_SUBVECTOR: {
    ...
    assert(
        (VT.isScalableVector() != N1VT.isScalableVector() ||
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT, SDValue N1, SDValue N2,
                              SDValue N3, const SDNodeFlags Flags) {
  assert(N1.getOpcode() != ISD::DELETED_NODE && N2.getOpcode() != ISD::DELETED_NODE &&
  ...
  case ISD::FMA: {
    assert(VT.isFloatingPoint() && "This operator only applies to FP types!");
    assert(N1.getValueType() == VT && N2.getValueType() == VT && N3.getValueType() == VT &&
  ...
  case ISD::INSERT_SUBVECTOR: {
    ...
    assert((VT.isScalableVector() != N2VT.isScalableVector() ||
            (N2VT.getVectorMinNumElements() + cast<ConstantSDNode>(N3)->getZExtValue()) <=
...
static SDValue getMemsetValue(SDValue Value, EVT VT, SelectionDAG &DAG, const SDLoc &dl) {
  ...
  if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(Value)) {
    ...
    return DAG.getConstantFP(APFloat(DAG.EVTToAPFloatSemantics(VT), Val), dl, VT);
  }
...
static SDValue getMemsetStringVal(EVT VT, const SDLoc &dl, SelectionDAG &DAG,
                                  const TargetLowering &TLI, const ConstantDataArraySlice &Slice) {
  ...
  if (Slice.Array == nullptr) {
    ...
    if (VT.isVector()) {
      ...
      return DAG.getNode(
          ISD::BITCAST, dl, VT,
          DAG.getConstant(0, dl, EVT::getVectorVT(*DAG.getContext(), EltVT, NumElts)));
    }
...
SDValue SelectionDAG::getMemBasePlusOffset(SDValue Base, TypeSize Offset, const SDLoc &DL,
                                           const SDNodeFlags Flags) {
  ...
  if (Offset.isScalable())
    Index = getVScale(DL, Base.getValueType(),
                      APInt(Base.getValueSizeInBits().getFixedValue(), Offset.getKnownMinValue()));
  else
...
SDValue SelectionDAG::getMemBasePlusOffset(SDValue Ptr, SDValue Offset, const SDLoc &DL,
                                           const SDNodeFlags Flags) {
...
static void chainLoadsAndStoresForMemcpy(SelectionDAG &DAG, const SDLoc &dl,
  ...
  for (unsigned i = From; i < To; ++i) {
    ...
    SDValue NewStore = DAG.getTruncStore(LoadToken, dl, ST->getValue(), ST->getBasePtr(),
                                         ST->getMemoryVT(), ST->getMemOperand());
    OutChains.push_back(NewStore);
  }
}

static SDValue getMemcpyLoadsAndStores(SelectionDAG &DAG, const SDLoc &dl, SDValue Chain,
                                       SDValue Dst, SDValue Src, uint64_t Size, Align Alignment,
                                       bool isVol, bool AlwaysInline, MachinePointerInfo DstPtrInfo,
  ...
  const MemOp Op = isZeroConstant ? MemOp::Set(Size, DstAlignCanChange, Alignment,
                                               /*IsZeroMemset*/ true, isVol)
                                  : MemOp::Copy(Size, DstAlignCanChange, Alignment, *SrcAlign,
                                                isVol, CopyFromConstant);
  if (!TLI.findOptimalMemOpLowering(MemOps, Limit, Op, DstPtrInfo.getAddrSpace(),
  ...
  bool isConstant =
      AA && SrcVal && AA->pointsToConstantMemory(MemoryLocation(SrcVal, Size, AAInfo));
  ...
  for (unsigned i = 0; i != NumMemOps; ++i) {
    ...
    if (!Store.getNode()) {
      ...
      Store = DAG.getTruncStore(
          Chain, dl, Value, DAG.getMemBasePlusOffset(Dst, TypeSize::Fixed(DstOff), dl),
  ...
  if (NumLdStInMemcpy) {
    ...
    if ((GluedLdStLimit <= 1) || !EnableMemCpyDAGOpt) {
      ...
      if (NumLdStInMemcpy <= GluedLdStLimit) {
        chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, 0, NumLdStInMemcpy, OutLoadChains,
                                     OutStoreChains);
      } else {
        ...
        for (unsigned cnt = 0; cnt < NumberLdChain; ++cnt) {
          ...
          chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, IndexFrom, IndexTo, OutLoadChains,
                                       OutStoreChains);
        ...
        if (RemainingLdStInMemcpy) {
          chainLoadsAndStoresForMemcpy(DAG, dl, OutChains, 0, RemainingLdStInMemcpy, OutLoadChains,
                                       OutStoreChains);
        }
      }
    }
...
static SDValue getMemmoveLoadsAndStores(SelectionDAG &DAG, const SDLoc &dl, SDValue Chain,
                                        SDValue Dst, SDValue Src, uint64_t Size, Align Alignment,
  ...
  if (!TLI.findOptimalMemOpLowering(MemOps, Limit,
                                    MemOp::Copy(Size, DstAlignCanChange, Alignment, *SrcAlign,
                                                /*IsVolatile*/ true),
  ...
  for (unsigned i = 0; i < NumMemOps; i++) {
    ...
    Value = DAG.getLoad(VT, dl, Chain, DAG.getMemBasePlusOffset(Src, TypeSize::Fixed(SrcOff), dl),
...
static SDValue getMemsetStores(SelectionDAG &DAG, const SDLoc &dl, SDValue Chain, SDValue Dst,
                               SDValue Src, uint64_t Size, Align Alignment, bool isVol,
  ...
  if (!TLI.findOptimalMemOpLowering(
          MemOps, Limit, MemOp::Set(Size, DstAlignCanChange, Alignment, IsZeroVal, isVol),
  ...
  for (unsigned i = 0; i < NumMemOps; i++) {
    ...
    SDValue Store =
        DAG.getStore(Chain, dl, Value, DAG.getMemBasePlusOffset(Dst, TypeSize::Fixed(DstOff), dl),
                     DstPtrInfo.getWithOffset(DstOff), Alignment,
                     isVol ? MachineMemOperand::MOVolatile : MachineMemOperand::MONone, NewAAInfo);
    OutChains.push_back(Store);
...
SDValue SelectionDAG::getMemcpy(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Src,
  ...
  if (ConstantSize) {
    ...
    SDValue Result =
        getMemcpyLoadsAndStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(), Alignment,
  ...
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemcpy(*this, dl, Chain, Dst, Src, Size, Alignment,
  ...
  if (AlwaysInline) {
    ...
    return getMemcpyLoadsAndStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(),
                                   Alignment, isVol, true, DstPtrInfo, SrcPtrInfo, AAInfo, AA);
  }
...
SDValue SelectionDAG::getAtomicMemcpy(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Src,
                                      SDValue Size, Type *SizeTy, unsigned ElemSz, bool isTailCall,
                                      MachinePointerInfo DstPtrInfo,
  ...
  CLI.setDebugLoc(dl)
      .setChain(Chain)
      .setLibCallee(
          TLI->getLibcallCallingConv(LibraryCall), Type::getVoidTy(*getContext()),
...
SDValue SelectionDAG::getMemmove(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Src,
  ...
  if (ConstantSize) {
    ...
    SDValue Result =
        getMemmoveLoadsAndStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(),
                                 Alignment, isVol, false, DstPtrInfo, SrcPtrInfo, AAInfo);
    if (Result.getNode())
      return Result;
  }
  ...
  // code. If the target chooses to do this, this is the next best.
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemmove(*this, dl, Chain, Dst, Src, Size, Alignment,
                                                   isVol, DstPtrInfo, SrcPtrInfo);
    if (Result.getNode())
      return Result;
  }
...
SDValue SelectionDAG::getAtomicMemmove(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Src,
                                       SDValue Size, Type *SizeTy, unsigned ElemSz, bool isTailCall,
                                       MachinePointerInfo DstPtrInfo,
                                       MachinePointerInfo SrcPtrInfo) {
  // Emit a library call.
...
SDValue SelectionDAG::getMemset(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Src,
                                SDValue Size, Align Alignment, bool isVol, bool AlwaysInline,
                                bool isTailCall, MachinePointerInfo DstPtrInfo,
                                const AAMDNodes &AAInfo) {
  ...
  if (ConstantSize) {
    ...
    SDValue Result = getMemsetStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(),
                                     Alignment, isVol, false, DstPtrInfo, AAInfo);
  ...
  // code. If the target chooses to do this, this is the next best.
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemset(*this, dl, Chain, Dst, Src, Size, Alignment,
  ...
  if (AlwaysInline) {
    assert(ConstantSize && "AlwaysInline requires a constant size!");
    SDValue Result = getMemsetStores(*this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(),
                                     Alignment, isVol, true, DstPtrInfo, AAInfo);
  ...
  if (SrcIsZero && BzeroName) {
    ...
    CLI.setLibCallee(TLI->getLibcallCallingConv(RTLIB::BZERO), Type::getVoidTy(Ctx),
...
SDValue SelectionDAG::getAtomicMemset(SDValue Chain, const SDLoc &dl, SDValue Dst, SDValue Value,
                                      SDValue Size, Type *SizeTy, unsigned ElemSz, bool isTailCall,
                                      MachinePointerInfo DstPtrInfo) {
  // Emit a library call.
  TargetLowering::ArgListTy Args;
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc &dl, EVT MemVT, SDVTList VTList,
  ...
  auto *N = newSDNode<AtomicSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), VTList, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getAtomicCmpSwap(unsigned Opcode, const SDLoc &dl, EVT MemVT, SDVTList VTs,
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc &dl, EVT MemVT, SDValue Chain,
  ...
  assert((Opcode == ISD::ATOMIC_LOAD_ADD || Opcode == ISD::ATOMIC_LOAD_SUB ||
          ...
          Opcode == ISD::ATOMIC_LOAD_FMIN || Opcode == ISD::ATOMIC_LOAD_UINC_WRAP ||
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc &dl, EVT MemVT, EVT VT, SDValue Chain,
...
SDValue SelectionDAG::getMemIntrinsicNode(unsigned Opcode, const SDLoc &dl, SDVTList VTList,
                                          ArrayRef<SDValue> Ops, EVT MemVT,
                                          MachinePointerInfo PtrInfo, Align Alignment,
                                          MachineMemOperand::Flags Flags, uint64_t Size,
                                          const AAMDNodes &AAInfo) {
...
SDValue SelectionDAG::getMemIntrinsicNode(unsigned Opcode, const SDLoc &dl, SDVTList VTList,
  ...
  if (VTList.VTs[VTList.NumVTs - 1] != MVT::Glue) {
    FoldingSetNodeID ID;
    AddNodeIDNode(ID, Opcode, VTList, Ops);
    ID.AddInteger(getSyntheticNodeSubclassData<MemIntrinsicSDNode>(Opcode, dl.getIROrder(), VTList,
                                                                   MemVT, MMO));
    ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
    ID.AddInteger(MMO->getFlags());
    ID.AddInteger(MemVT.getRawBits());
    ...
    N = newSDNode<MemIntrinsicSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), VTList, MemVT,
                                      MMO);
    createOperands(N, Ops);
...
SDValue SelectionDAG::getLifetimeNode(bool IsStart, const SDLoc &dl, SDValue Chain, int FrameIndex,
                                      int64_t Size, int64_t Offset) {
  const unsigned Opcode = IsStart ? ISD::LIFETIME_START : ISD::LIFETIME_END;
  const auto VTs = getVTList(MVT::Other);
  SDValue Ops[2] = {
      Chain,
      getFrameIndex(FrameIndex, getTargetLoweringInfo().getFrameIndexTy(getDataLayout()), true)};
  ...
  LifetimeSDNode *N =
      newSDNode<LifetimeSDNode>(Opcode, dl.getIROrder(), dl.getDebugLoc(), VTs, Size, Offset);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
  SDValue V(N, 0);
...
SDValue SelectionDAG::getPseudoProbeNode(const SDLoc &Dl, SDValue Chain, uint64_t Guid,
  ...
  auto *N = newSDNode<PseudoProbeSDNode>(Opcode, Dl.getIROrder(), Dl.getDebugLoc(), VTs, Guid,
                                         Index, Attr);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
...
static MachinePointerInfo InferPointerInfo(const MachinePointerInfo &Info, SelectionDAG &DAG,
                                           SDValue Ptr, int64_t Offset = 0) {
...
SDValue SelectionDAG::getLoad(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
  ...
  MachineFunction &MF = getMachineFunction();
  MachineMemOperand *MMO =
      MF.getMachineMemOperand(PtrInfo, MMOFlags, Size, Alignment, AAInfo, Ranges);
...
SDValue SelectionDAG::getLoad(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
  ...
  SDVTList VTs =
      Indexed ? getVTList(VT, Ptr.getValueType(), MVT::Other) : getVTList(VT, MVT::Other);
  SDValue Ops[] = {Chain, Ptr, Offset};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::LOAD, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(
      getSyntheticNodeSubclassData<LoadSDNode>(dl.getIROrder(), VTs, AM, ExtType, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, dl, IP)) {
...
SDValue SelectionDAG::getLoad(EVT VT, const SDLoc &dl, SDValue Chain, SDValue Ptr,
                              MachinePointerInfo PtrInfo, MaybeAlign Alignment,
                              MachineMemOperand::Flags MMOFlags, const AAMDNodes &AAInfo,
                              const MDNode *Ranges) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, PtrInfo, VT,
...
SDValue SelectionDAG::getExtLoad(ISD::LoadExtType ExtType, const SDLoc &dl, EVT VT, SDValue Chain,
                                 SDValue Ptr, MachinePointerInfo PtrInfo, EVT MemVT,
                                 MaybeAlign Alignment, MachineMemOperand::Flags MMOFlags,
                                 const AAMDNodes &AAInfo) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoad(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, PtrInfo, MemVT, Alignment,
                 MMOFlags, AAInfo);
}
...
SDValue SelectionDAG::getIndexedLoad(SDValue OrigLoad, const SDLoc &dl, SDValue Base,
                                     SDValue Offset, ISD::MemIndexedMode AM) {
  LoadSDNode *LD = cast<LoadSDNode>(OrigLoad);
  assert(LD->getOffset().isUndef() && "Load is already a indexed load!");
  // Don't propagate the invariant or dereferenceable flags.
  auto MMOFlags = LD->getMemOperand()->getFlags() &
                  ~(MachineMemOperand::MOInvariant | MachineMemOperand::MODereferenceable);
  return getLoad(AM, LD->getExtensionType(), OrigLoad.getValueType(), dl, LD->getChain(), Base,
                 Offset, LD->getPointerInfo(), LD->getMemoryVT(), LD->getAlign(), MMOFlags,
                 LD->getAAInfo());
}

SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
                               MachinePointerInfo PtrInfo, Align Alignment,
                               MachineMemOperand::Flags MMOFlags, const AAMDNodes &AAInfo) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  ...
  MachineFunction &MF = getMachineFunction();
  uint64_t Size = MemoryLocation::getSizeOrUnknown(Val.getValueType().getStoreSize());
...
SDValue SelectionDAG::getStore(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
  ...
  ID.AddInteger(getSyntheticNodeSubclassData<StoreSDNode>(dl.getIROrder(), VTs, ISD::UNINDEXED,
                                                          false, VT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  ...
  auto *N = newSDNode<StoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, ISD::UNINDEXED, false,
                                   VT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getTruncStore(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
  ...
  MachineMemOperand *MMO = MF.getMachineMemOperand(
      PtrInfo, MMOFlags, MemoryLocation::getSizeOrUnknown(SVT.getStoreSize()), Alignment, AAInfo);
...
SDValue SelectionDAG::getTruncStore(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
  ...
  auto *N = newSDNode<StoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, ISD::UNINDEXED, true,
                                   SVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedStore(SDValue OrigStore, const SDLoc &dl, SDValue Base,
                                      SDValue Offset, ISD::MemIndexedMode AM) {
  ...
  auto *N = newSDNode<StoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM,
                                   ST->isTruncatingStore(), ST->getMemoryVT(), ST->getMemOperand());
  createOperands(N, Ops);
...
SDValue SelectionDAG::getLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
                                const SDLoc &dl, SDValue Chain, SDValue Ptr, SDValue Offset,
...
SDValue SelectionDAG::getLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
                                const SDLoc &dl, SDValue Chain, SDValue Ptr, SDValue Offset,
                                SDValue Mask, SDValue EVL, EVT MemVT, MachineMemOperand *MMO,
                                bool IsExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed load with an offset!");
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPLoadSDNode>(dl.getIROrder(), VTs, AM, ExtType,
  ...
  auto *N = newSDNode<VPLoadSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ExtType,
                                    IsExpanding, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getLoadVP(EVT VT, const SDLoc &dl, SDValue Chain, SDValue Ptr, SDValue Mask,
                                SDValue EVL, MachinePointerInfo PtrInfo, MaybeAlign Alignment,
                                MachineMemOperand::Flags MMOFlags, const AAMDNodes &AAInfo,
                                const MDNode *Ranges, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, Mask, EVL, PtrInfo,
                   VT, Alignment, MMOFlags, AAInfo, Ranges, IsExpanding);
}

SDValue SelectionDAG::getLoadVP(EVT VT, const SDLoc &dl, SDValue Chain, SDValue Ptr, SDValue Mask,
                                SDValue EVL, MachineMemOperand *MMO, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, dl, Chain, Ptr, Undef, Mask, EVL, VT, MMO,
                   IsExpanding);
}

SDValue SelectionDAG::getExtLoadVP(ISD::LoadExtType ExtType, const SDLoc &dl, EVT VT, SDValue Chain,
                                   SDValue Ptr, SDValue Mask, SDValue EVL,
                                   MachinePointerInfo PtrInfo, EVT MemVT, MaybeAlign Alignment,
                                   MachineMemOperand::Flags MMOFlags, const AAMDNodes &AAInfo,
                                   bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, Mask, EVL, PtrInfo, MemVT,
                   Alignment, MMOFlags, AAInfo, nullptr, IsExpanding);
}

SDValue SelectionDAG::getExtLoadVP(ISD::LoadExtType ExtType, const SDLoc &dl, EVT VT, SDValue Chain,
                                   SDValue Ptr, SDValue Mask, SDValue EVL, EVT MemVT,
                                   MachineMemOperand *MMO, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getLoadVP(ISD::UNINDEXED, ExtType, VT, dl, Chain, Ptr, Undef, Mask, EVL, MemVT, MMO,
                   IsExpanding);
}

SDValue SelectionDAG::getIndexedLoadVP(SDValue OrigLoad, const SDLoc &dl, SDValue Base,
                                       SDValue Offset, ISD::MemIndexedMode AM) {
  ...
  auto MMOFlags = LD->getMemOperand()->getFlags() &
                  ~(MachineMemOperand::MOInvariant | MachineMemOperand::MODereferenceable);
  return getLoadVP(AM, LD->getExtensionType(), OrigLoad.getValueType(), dl, LD->getChain(), Base,
                   Offset, LD->getMask(), LD->getVectorLength(), LD->getPointerInfo(),
                   LD->getMemoryVT(), LD->getAlign(), MMOFlags, LD->getAAInfo(), nullptr,
                   LD->isExpandingLoad());
}

SDValue SelectionDAG::getStoreVP(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPStoreSDNode>(dl.getIROrder(), VTs, AM, IsTruncating,
  ...
  auto *N = newSDNode<VPStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, IsTruncating,
                                     IsCompressing, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getTruncStoreVP(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
                                      SDValue Mask, SDValue EVL, MachinePointerInfo PtrInfo,
                                      EVT SVT, Align Alignment, MachineMemOperand::Flags MMOFlags,
...
SDValue SelectionDAG::getTruncStoreVP(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Ptr,
                                      SDValue Mask, SDValue EVL, EVT SVT, MachineMemOperand *MMO,
                                      bool IsCompressing) {
  EVT VT = Val.getValueType();

  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  if (VT == SVT)
    return getStoreVP(Chain, dl, Val, Ptr, getUNDEF(Ptr.getValueType()), Mask, EVL, VT, MMO,
                      ISD::UNINDEXED,
  ...
  AddNodeIDNode(ID, ISD::VP_STORE, VTs, Ops);
  ID.AddInteger(SVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPStoreSDNode>(dl.getIROrder(), VTs, ISD::UNINDEXED,
                                                            true, IsCompressing, SVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, dl, IP)) {
  ...
  auto *N = newSDNode<VPStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, ISD::UNINDEXED, true,
...
SDValue SelectionDAG::getIndexedStoreVP(SDValue OrigStore, const SDLoc &dl, SDValue Base,
                                        SDValue Offset, ISD::MemIndexedMode AM) {
  ...
  auto *N =
      newSDNode<VPStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ST->isTruncatingStore(),
...
SDValue SelectionDAG::getStridedLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
                                       const SDLoc &DL, SDValue Chain, SDValue Ptr, SDValue Offset,
  ...
  MachineMemOperand *MMO =
      MF.getMachineMemOperand(PtrInfo, MMOFlags, Size, Alignment, AAInfo, Ranges);
  return getStridedLoadVP(AM, ExtType, VT, DL, Chain, Ptr, Offset, Stride, Mask, EVL, MemVT, MMO,
                          IsExpanding);
}

SDValue SelectionDAG::getStridedLoadVP(ISD::MemIndexedMode AM, ISD::LoadExtType ExtType, EVT VT,
                                       const SDLoc &DL, SDValue Chain, SDValue Ptr, SDValue Offset,
                                       SDValue Stride, SDValue Mask, SDValue EVL, EVT MemVT,
                                       MachineMemOperand *MMO, bool IsExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed load with an offset!");
  ...
  ID.AddInteger(VT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<VPStridedLoadSDNode>(DL.getIROrder(), VTs, AM, ExtType,
                                                                  IsExpanding, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ...
  auto *N = newSDNode<VPStridedLoadSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, AM, ExtType,
                                           IsExpanding, MemVT, MMO);
  createOperands(N, Ops);
  CSEMap.InsertNode(N, IP);
  InsertNode(N);
...
SDValue SelectionDAG::getStridedLoadVP(EVT VT, const SDLoc &DL, SDValue Chain, SDValue Ptr,
                                       SDValue Stride, SDValue Mask, SDValue EVL,
                                       MachinePointerInfo PtrInfo, MaybeAlign Alignment,
                                       MachineMemOperand::Flags MMOFlags, const AAMDNodes &AAInfo,
                                       const MDNode *Ranges, bool IsExpanding) {
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, DL, Chain, Ptr, Undef, Stride, Mask,
                          EVL, PtrInfo, VT, Alignment, MMOFlags, AAInfo, Ranges, IsExpanding);
}
...
SDValue SelectionDAG::getStridedLoadVP(EVT VT, const SDLoc &DL, SDValue Chain, SDValue Ptr,
  ...
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ISD::NON_EXTLOAD, VT, DL, Chain, Ptr, Undef, Stride, Mask,
                          EVL, VT, MMO, IsExpanding);
}

SDValue SelectionDAG::getExtStridedLoadVP(ISD::LoadExtType ExtType, const SDLoc &DL, EVT VT,
  ...
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ExtType, VT, DL, Chain, Ptr, Undef, Stride, Mask, EVL,
                          PtrInfo, MemVT, Alignment, MMOFlags, AAInfo, nullptr, IsExpanding);
}
...
SDValue SelectionDAG::getExtStridedLoadVP(ISD::LoadExtType ExtType, const SDLoc &DL, EVT VT,
  ...
  SDValue Undef = getUNDEF(Ptr.getValueType());
  return getStridedLoadVP(ISD::UNINDEXED, ExtType, VT, DL, Chain, Ptr, Undef, Stride, Mask, EVL,
                          MemVT, MMO, IsExpanding);
}

SDValue SelectionDAG::getIndexedStridedLoadVP(SDValue OrigLoad, const SDLoc &DL, SDValue Base,
                                              SDValue Offset, ISD::MemIndexedMode AM) {
  auto *SLD = cast<VPStridedLoadSDNode>(OrigLoad);
  assert(SLD->getOffset().isUndef() && "Strided load is already a indexed load!");
  // Don't propagate the invariant or dereferenceable flags.
  auto MMOFlags = SLD->getMemOperand()->getFlags() &
                  ~(MachineMemOperand::MOInvariant | MachineMemOperand::MODereferenceable);
  return getStridedLoadVP(AM, SLD->getExtensionType(), OrigLoad.getValueType(), DL, SLD->getChain(),
...
SDValue SelectionDAG::getStridedStoreVP(SDValue Chain, const SDLoc &DL, SDValue Val, SDValue Ptr,
                                        SDValue Offset, SDValue Stride, SDValue Mask, SDValue EVL,
  ...
  auto *N = newSDNode<VPStridedStoreSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, AM,
...
SDValue SelectionDAG::getTruncStridedStoreVP(SDValue Chain, const SDLoc &DL, SDValue Val,
  ...
  MachineFunction &MF = getMachineFunction();
  MachineMemOperand *MMO =
      MF.getMachineMemOperand(PtrInfo, MMOFlags, MemoryLocation::UnknownSize, Alignment, AAInfo);
  return getTruncStridedStoreVP(Chain, DL, Val, Ptr, Stride, Mask, EVL, SVT, MMO, IsCompressing);
}

SDValue SelectionDAG::getTruncStridedStoreVP(SDValue Chain, const SDLoc &DL, SDValue Val,
                                             SDValue Ptr, SDValue Stride, SDValue Mask, SDValue EVL,
                                             EVT SVT, MachineMemOperand *MMO, bool IsCompressing) {
  EVT VT = Val.getValueType();

  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  if (VT == SVT)
    return getStridedStoreVP(Chain, DL, Val, Ptr, getUNDEF(Ptr.getValueType()), Stride, Mask, EVL,
                             VT, MMO, ISD::UNINDEXED,
                             /*IsTruncating*/ false, IsCompressing);
  ...
  auto *N = newSDNode<VPStridedStoreSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, ISD::UNINDEXED,
                                            true, IsCompressing, SVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedStridedStoreVP(SDValue OrigStore, const SDLoc &DL, SDValue Base,
                                               SDValue Offset, ISD::MemIndexedMode AM) {
  auto *SST = cast<VPStridedStoreSDNode>(OrigStore);
  assert(SST->getOffset().isUndef() && "Strided store is already an indexed store!");
  SDVTList VTs = getVTList(Base.getValueType(), MVT::Other);
  SDValue Ops[] = {SST->getChain(), SST->getValue(),       Base, Offset, SST->getStride(),
  ...
  auto *N = newSDNode<VPStridedStoreSDNode>(DL.getIROrder(), DL.getDebugLoc(), VTs, AM,
                                            ...
                                            SST->getMemoryVT(), SST->getMemOperand());
  createOperands(N, Ops);
...
SDValue SelectionDAG::getGatherVP(SDVTList VTs, EVT VT, const SDLoc &dl, ArrayRef<SDValue> Ops,
  ...
  ID.AddInteger(VT.getRawBits());
  ID.AddInteger(
      getSyntheticNodeSubclassData<VPGatherSDNode>(dl.getIROrder(), VTs, VT, MMO, IndexType));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, dl, IP)) {
...
SDValue SelectionDAG::getScatterVP(SDVTList VTs, EVT VT, const SDLoc &dl, ArrayRef<SDValue> Ops,
                                   MachineMemOperand *MMO, ISD::MemIndexType IndexType) {
  ...
  ID.AddInteger(VT.getRawBits());
  ID.AddInteger(
      getSyntheticNodeSubclassData<VPScatterSDNode>(dl.getIROrder(), VTs, VT, MMO, IndexType));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  if (SDNode *E = FindNodeOrInsertPos(ID, dl, IP)) {
...
SDValue SelectionDAG::getMaskedLoad(EVT VT, const SDLoc &dl, SDValue Chain, SDValue Base,
                                    SDValue Offset, SDValue Mask, SDValue PassThru, EVT MemVT,
                                    MachineMemOperand *MMO, ISD::MemIndexedMode AM,
                                    ISD::LoadExtType ExtTy, bool isExpanding) {
  bool Indexed = AM != ISD::UNINDEXED;
  assert((Indexed || Offset.isUndef()) && "Unindexed masked load with an offset!");
  SDVTList VTs =
      Indexed ? getVTList(VT, Base.getValueType(), MVT::Other) : getVTList(VT, MVT::Other);
  SDValue Ops[] = {Chain, Base, Offset, Mask, PassThru};
  FoldingSetNodeID ID;
  AddNodeIDNode(ID, ISD::MLOAD, VTs, Ops);
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedLoadSDNode>(dl.getIROrder(), VTs, AM, ExtTy,
                                                               isExpanding, MemVT, MMO));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  ...
  auto *N = newSDNode<MaskedLoadSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, ExtTy,
                                        isExpanding, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedMaskedLoad(SDValue OrigLoad, const SDLoc &dl, SDValue Base,
                                           SDValue Offset, ISD::MemIndexedMode AM) {
  MaskedLoadSDNode *LD = cast<MaskedLoadSDNode>(OrigLoad);
  assert(LD->getOffset().isUndef() && "Masked load is already a indexed load!");
  return getMaskedLoad(OrigLoad.getValueType(), dl, LD->getChain(), Base, Offset, LD->getMask(),
                       LD->getPassThru(), LD->getMemoryVT(), LD->getMemOperand(), AM,
                       LD->getExtensionType(), LD->isExpandingLoad());
}

SDValue SelectionDAG::getMaskedStore(SDValue Chain, const SDLoc &dl, SDValue Val, SDValue Base,
                                     SDValue Offset, SDValue Mask, EVT MemVT,
                                     MachineMemOperand *MMO, ISD::MemIndexedMode AM,
                                     bool IsTruncating, bool IsCompressing) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  bool Indexed = AM != ISD::UNINDEXED;
  ...
  auto *N = newSDNode<MaskedStoreSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, AM, IsTruncating,
                                         IsCompressing, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getIndexedMaskedStore(SDValue OrigStore, const SDLoc &dl, SDValue Base,
                                            SDValue Offset, ISD::MemIndexedMode AM) {
  MaskedStoreSDNode *ST = cast<MaskedStoreSDNode>(OrigStore);
  assert(ST->getOffset().isUndef() && "Masked store is already a indexed store!");
  return getMaskedStore(ST->getChain(), dl, ST->getValue(), Base, Offset, ST->getMask(),
                        ST->getMemoryVT(), ST->getMemOperand(), AM, ST->isTruncatingStore(),
...
SDValue SelectionDAG::getMaskedGather(SDVTList VTs, EVT MemVT, const SDLoc &dl,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedGatherSDNode>(dl.getIROrder(), VTs, MemVT, MMO,
                                                                 IndexType, ExtTy));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  ...
  auto *N = newSDNode<MaskedGatherSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO,
                                          IndexType, ExtTy);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getMaskedScatter(SDVTList VTs, EVT MemVT, const SDLoc &dl,
  ...
  ID.AddInteger(MemVT.getRawBits());
  ID.AddInteger(getSyntheticNodeSubclassData<MaskedScatterSDNode>(dl.getIROrder(), VTs, MemVT, MMO,
                                                                  IndexType, IsTrunc));
  ID.AddInteger(MMO->getPointerInfo().getAddrSpace());
  ID.AddInteger(MMO->getFlags());
  void *IP = nullptr;
  ...
  auto *N = newSDNode<MaskedScatterSDNode>(dl.getIROrder(), dl.getDebugLoc(), VTs, MemVT, MMO,
                                           IndexType, IsTrunc);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getGetFPEnv(SDValue Chain, const SDLoc &dl, SDValue Ptr, EVT MemVT,
                                  MachineMemOperand *MMO) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  SDVTList VTs = getVTList(MVT::Other);
  SDValue Ops[] = {Chain, Ptr};
  ...
  auto *N = newSDNode<FPStateAccessSDNode>(ISD::GET_FPENV_MEM, dl.getIROrder(), dl.getDebugLoc(),
                                           VTs, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getSetFPEnv(SDValue Chain, const SDLoc &dl, SDValue Ptr, EVT MemVT,
                                  MachineMemOperand *MMO) {
  assert(Chain.getValueType() == MVT::Other && "Invalid chain type");
  SDVTList VTs = getVTList(MVT::Other);
  SDValue Ops[] = {Chain, Ptr};
  ...
  auto *N = newSDNode<FPStateAccessSDNode>(ISD::SET_FPENV_MEM, dl.getIROrder(), dl.getDebugLoc(),
                                           VTs, MemVT, MMO);
  createOperands(N, Ops);
...
SDValue SelectionDAG::getVAArg(EVT VT, const SDLoc &dl, SDValue Chain, SDValue Ptr, SDValue SV,
                               unsigned Align) {
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT, ArrayRef<SDValue> Ops,
  ...
  case ISD::SELECT_CC:
    ...
    assert((!Ops[0].getValueType().isVector() ||
            Ops[0].getValueType().getVectorElementCount() == VT.getVectorElementCount()) &&
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, ArrayRef<EVT> ResultTys,
                              ArrayRef<SDValue> Ops) {
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, SDVTList VTList,
  ...
  case ISD::UMUL_LOHI: {
    ...
    assert(VTList.VTs[0].isInteger() && VTList.VTs[0] == VTList.VTs[1] &&
           VTList.VTs[0] == Ops[0].getValueType() && VTList.VTs[0] == Ops[1].getValueType() &&
           "Binary operator types must match!");
    break;
  }
  case ISD::FFREXP: {
  ...
  case ISD::STRICT_FP_EXTEND:
    ...
    assert(VTList.VTs[0].isVector() == Ops[1].getValueType().isVector() &&
           ...
           "type is vector!");
    assert((!VTList.VTs[0].isVector() || VTList.VTs[0].getVectorElementCount() ==
  ...
  case ISD::STRICT_FP_ROUND:
    ...
    assert((!VTList.VTs[0].isVector() || VTList.VTs[0].getVectorElementCount() ==
           ...
           "Vector element count mismatch!");
    assert(VTList.VTs[0].isFloatingPoint() && Ops[1].getValueType().isFloatingPoint() &&
           VTList.VTs[0].bitsLT(Ops[1].getValueType()) && isa<ConstantSDNode>(Ops[2]) &&
...
SDVTList SelectionDAG::getVTList(EVT VT) { return makeVTList(SDNode::getValueTypeList(VT), 1); }
...
SDNode *SelectionDAG::SelectNodeTo(SDNode *N, unsigned MachineOpc, EVT VT1, EVT VT2, EVT VT3,
                                   ArrayRef<SDValue> Ops) {
  SDVTList VTs = getVTList(VT1, VT2, VT3);
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
}

SDNode *SelectionDAG::SelectNodeTo(SDNode *N, unsigned MachineOpc, EVT VT1, EVT VT2, SDValue Op1,
                                   SDValue Op2) {
  SDVTList VTs = getVTList(VT1, VT2);
  SDValue Ops[] = {Op1, Op2};
  return SelectNodeTo(N, MachineOpc, VTs, Ops);
...
MachineSDNode *SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc &dl, EVT VT1, EVT VT2,
...
MachineSDNode *SelectionDAG::getMachineNode(unsigned Opcode, const SDLoc &DL, SDVTList VTs,
                                            ArrayRef<SDValue> Ops) {
...
SDValue SelectionDAG::getTargetInsertSubreg(int SRIdx, const SDLoc &DL, EVT VT, SDValue Operand,
                                            SDValue Subreg) {
  SDValue SRIdxVal = getTargetConstant(SRIdx, DL, MVT::i32);
...
///
/// SDNode
SDDbgValue *SelectionDAG::getDbgValue(DIVariable *Var, DIExpression *Expr, SDNode *N, unsigned R,
                                      bool IsIndirect, const DebugLoc &DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) &&
         "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(), Var, Expr,
                                              SDDbgOperand::fromNode(N, R), {}, IsIndirect, DL, O,
                                              /*IsVariadic=*/false);
}

/// Constant
SDDbgValue *SelectionDAG::getConstantDbgValue(DIVariable *Var, DIExpression *Expr, const Value *C,
...
/// FrameIndex
SDDbgValue *SelectionDAG::getFrameIndexDbgValue(DIVariable *Var, DIExpression *Expr, unsigned FI,
                                                bool IsIndirect, const DebugLoc &DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) &&
         "Expected inlined-at fields to agree");
...
SDDbgValue *SelectionDAG::getFrameIndexDbgValue(DIVariable *Var, DIExpression *Expr, unsigned FI,
  ...
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) &&
         "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc())
      SDDbgValue(DbgInfo->getAlloc(), Var, Expr, SDDbgOperand::fromFrameIdx(FI), Dependencies,
                 IsIndirect, DL, O,
                 /*IsVariadic=*/false);
}

/// VReg
SDDbgValue *SelectionDAG::getVRegDbgValue(DIVariable *Var, DIExpression *Expr, unsigned VReg,
                                          bool IsIndirect, const DebugLoc &DL, unsigned O) {
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) &&
         "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc()) SDDbgValue(DbgInfo->getAlloc(), Var, Expr,
                                              SDDbgOperand::fromVReg(VReg), {}, IsIndirect, DL, O,
                                              /*IsVariadic=*/false);
}
...
SDDbgValue *SelectionDAG::getDbgValueList(DIVariable *Var, DIExpression *Expr,
  ...
  assert(cast<DILocalVariable>(Var)->isValidLocationForIntrinsic(DL) &&
         "Expected inlined-at fields to agree");
  return new (DbgInfo->getAlloc())
      SDDbgValue(DbgInfo->getAlloc(), Var, Expr, Locs, Dependencies, IsIndirect, DL, O, IsVariadic);
}

void SelectionDAG::transferDbgValues(SDValue From, SDValue To, unsigned OffsetInBits,
                                     unsigned SizeInBits, bool InvalidateDbg) {
  ...
  for (SDDbgValue *Dbg : GetDbgValues(FromNode)) {
    ...
    SDDbgValue *Clone = getDbgValueList(
        Var, Expr, NewLocOps, AdditionalDependencies, Dbg->isIndirect(), Dbg->getDebugLoc(),
        std::max(ToNode->getIROrder(), Dbg->getOrder()), Dbg->isVariadic());
...
void SelectionDAG::salvageDebugInfo(SDNode &N) {
  ...
  for (auto *DV : GetDbgValues(&N)) {
    ...
    case ISD::ADD:
      ...
      if (!isa<ConstantSDNode>(N0) && isa<ConstantSDNode>(N1)) {
        ...
        SDDbgValue *Clone =
            getDbgValueList(DV->getVariable(), DIExpr, NewLocOps, AdditionalDependencies,
                            DV->isIndirect(), DV->getDebugLoc(), DV->getOrder(), DV->isVariadic());
        ClonedDVs.push_back(Clone);
        DV->setIsInvalidated();
        DV->setIsEmitted();
  ...
  for (SDDbgValue *Dbg : ClonedDVs) {
    assert(!Dbg->getSDNodes().empty() && "Salvaged DbgValue should depend on a new SDNode");
    AddDbgValue(Dbg, false);
  }
}
...
void SelectionDAG::ReplaceAllUsesOfValuesWith(const SDValue *From, const SDValue *To,
                                              unsigned Num) {
  // Handle the simple, trivial case efficiently.
  if (Num == 1)
  ...
  for (unsigned i = 0; i != Num; ++i) {
    ...
    for (SDNode::use_iterator UI = FromNode->use_begin(), E = FromNode->use_end(); UI != E; ++UI) {
      SDUse &Use = UI.getUse();
...
SDValue SelectionDAG::makeEquivalentMemoryOrdering(SDValue OldChain, SDValue NewMemOpChain) {
  ...
  SDValue TokenFactor =
      getNode(ISD::TokenFactor, SDLoc(OldChain), MVT::Other, OldChain, NewMemOpChain);
...
bool llvm::isNeutralConstant(unsigned Opcode, SDNodeFlags Flags, SDValue V, unsigned OperandNo) {
  ...
  if (auto *Const = isConstOrConstSplat(V)) {
    ...
    switch (Opcode) {
    case ISD::FADD:
      return ConstFP->isZero() && (Flags.hasNoSignedZeros() || ConstFP->isNegative());
    case ISD::FSUB:
...
ConstantSDNode *llvm::isConstOrConstSplat(SDValue N, bool AllowUndefs, bool AllowTruncation) {
  ...
  APInt DemandedElts =
      VT.isFixedLengthVector() ? APInt::getAllOnes(VT.getVectorMinNumElements()) : APInt(1, 1);
...
ConstantFPSDNode *llvm::isConstOrConstSplatFP(SDValue N, const APInt &DemandedElts,
                                              bool AllowUndefs) {
...
GlobalAddressSDNode::GlobalAddressSDNode(unsigned Opc, unsigned Order, const DebugLoc &DL,
                                         const GlobalValue *GA, EVT VT, int64_t o, unsigned TF)
    : SDNode(Opc, Order, DL, getSDVTList(VT)), Offset(o), TargetFlags(TF) {
  TheGlobal = GA;
}

AddrSpaceCastSDNode::AddrSpaceCastSDNode(unsigned Order, const DebugLoc &dl, EVT VT, unsigned SrcAS,
                                         unsigned DestAS)
    : SDNode(ISD::ADDRSPACECAST, Order, dl, getSDVTList(VT)), SrcAddrSpace(SrcAS),
      DestAddrSpace(DestAS) {}

MemSDNode::MemSDNode(unsigned Opc, unsigned Order, const DebugLoc &dl, SDVTList VTs, EVT memvt,
                     MachineMemOperand *mmo)
  ...
  assert(memvt.getStoreSize().getKnownMinValue() <= MMO->getSize() && "Size mismatch!");
}
...
void SDNode::intersectFlagsWith(const SDNodeFlags Flags) { this->Flags.intersectWith(Flags); }

SDValue SelectionDAG::matchBinOpReduction(SDNode *Extract, ISD::NodeType &BinOp,
  ...
  if (llvm::none_of(CandidateBinOps,
                    [Op](ISD::NodeType BinOp) { return Op.getOpcode() == unsigned(BinOp); }))
    return SDValue();
  ...
  if (N->getNumValues() == 2) {
    ...
    for (i = 0; i != NE; ++i) {
      for (unsigned j = 0, e = N->getNumOperands(); j != e; ++j) {
        ...
        Operands[j] = getNode(ISD::EXTRACT_VECTOR_ELT, dl, OperandEltVT, Operand,
...
std::pair<SDValue, SDValue> SelectionDAG::UnrollVectorOverflowOp(SDNode *N, unsigned ResNE) {
  ...
  assert((Opcode == ISD::UADDO || Opcode == ISD::SADDO || Opcode == ISD::USUBO ||
          Opcode == ISD::SSUBO || Opcode == ISD::UMULO || Opcode == ISD::SMULO) &&
  ...
  for (unsigned i = 0; i < NE; ++i) {
    ...
    SDValue Ov = getSelect(dl, OvEltVT, Res.getValue(1), getBoolConstant(true, dl, OvEltVT, ResVT),
...
bool SelectionDAG::areNonVolatileConsecutiveLoads(LoadSDNode *LD, LoadSDNode *Base, unsigned Bytes,
                                                  int Dist) const {
...
std::pair<SDValue, SDValue> SelectionDAG::SplitScalar(const SDValue &N, const SDLoc &DL,
...
std::pair<EVT, EVT> SelectionDAG::GetDependentSplitDestVTs(const EVT &VT, const EVT &EnvVT,
...
/// low/high part.
std::pair<SDValue, SDValue> SelectionDAG::SplitVector(const SDValue &N, const SDLoc &DL,
                                                      const EVT &LoVT, const EVT &HiVT) {
  ...
  SDValue Lo, Hi;
  Lo = getNode(ISD::EXTRACT_SUBVECTOR, DL, LoVT, N, getVectorIdxConstant(0, DL));
...
std::pair<SDValue, SDValue> SelectionDAG::SplitEVL(SDValue N, EVT VecVT, const SDLoc &DL) {
  ...
  SDValue HalfNumElts = VecVT.isFixedLengthVector()
...
void SelectionDAG::ExtractVectorElements(SDValue Op, SmallVectorImpl<SDValue> &Args, unsigned Start,
                                         unsigned Count, EVT EltVT) {
  EVT VT = Op.getValueType();
  if (Count == 0)
  ...
  for (unsigned i = Start, e = Start + Count; i != e; ++i) {
    Args.push_back(getNode(ISD::EXTRACT_VECTOR_ELT, SL, EltVT, Op, getVectorIdxConstant(i, SL)));
  }
}
...
ConstantSDNode *BuildVectorSDNode::getConstantSplatNode(const APInt &DemandedElts,
                                                        BitVector *UndefElements) const {
...
ConstantFPSDNode *BuildVectorSDNode::getConstantFPSplatNode(const APInt &DemandedElts,
                                                            BitVector *UndefElements) const {
...
bool BuildVectorSDNode::getConstantRawBits(bool IsLittleEndian, unsigned DstEltSizeInBits,
  ...
  for (unsigned I = 0; I != NumSrcOps; ++I) {
    ...
    SrcBitElements[I] =
        CInt ? CInt->getAPIntValue().trunc(SrcEltSizeInBits) : CFP->getValueAPF().bitcastToAPInt();
  }

  // Recast to dst width.
  recastRawBits(IsLittleEndian, DstEltSizeInBits, RawBitElements, SrcBitElements, UndefElements,
                SrcUndeElements);
  return true;
}

void BuildVectorSDNode::recastRawBits(bool IsLittleEndian, unsigned DstEltSizeInBits,
...
void SelectionDAG::createOperands(SDNode *Node, ArrayRef<SDValue> Vals) {
  ...
  SDUse *Ops =
      OperandRecycler.allocate(ArrayRecycler<SDUse>::Capacity::get(Vals.size()), OperandAllocator);
...
SDValue SelectionDAG::getNeutralElement(unsigned Opcode, const SDLoc &DL, EVT VT,
                                        SDNodeFlags Flags) {
  switch (Opcode) {
  default:
    return SDValue();
  ...
  case ISD::FMAXIMUM: {
    ...
    const fltSemantics &Semantics = EVTToAPFloatSemantics(VT);
    APFloat NeutralAF =
        !Flags.hasNoInfs() ? APFloat::getInf(Semantics) : APFloat::getLargest(Semantics);
...
SDValue SelectionDAG::makeStateFunctionCall(unsigned LibFunc, SDValue Ptr, SDValue InChain,
                                            const SDLoc &DLoc) {
  ...
  CLI.setDebugLoc(DLoc).setChain(InChain).setLibCallee(
      TLI->getLibcallCallingConv(LC), Type::getVoidTy(*getContext()), Callee, std::move(Args));
...
#ifndef NDEBUG
static void checkForCyclesHelper(const SDNode *N, SmallPtrSetImpl<const SDNode *> &Visited,
